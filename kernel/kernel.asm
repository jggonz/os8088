...
; three bodies do not exist in the small build. cw_vid_disp_init was OUTSIDE
; the guard while its two siblings were inside it, which broke `make small`
; outright - the assembly stops at an undefined symbol. Its one caller
; (drv_vid_apply) is itself KERN_BIG-guarded, so nothing but the shim was
; wrong and nothing in the big build moves.
