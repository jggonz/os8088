/* A STUB apps/cc/os88.h for C64's host harness (apps/c64/hosttest/
 * c64uitest.c). Same names, same shapes, no poisoning of long/float (the host
 * needs printf). Only what c64.c and its parts use, and IT GROWS WITH EVERY
 * API THE PROGRAM TOUCHES, in the same edit as the thunk (LESSONS.md 7): a
 * shim added to the package without its stub here fails to LINK, which is the
 * failure you want.
 *
 * The MODEL OF THE GLASS is not here - it is in c64uitest.c, which defines
 * every function below. This file is only the declarations, so that c64.c
