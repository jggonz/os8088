 ...
 * SDK a package actually uses (SPEC.md 67). It is deliberately small and
 ...
 *     silently wrong here and tools/cc8086.py refuses it (67.5);
 *   - nothing is copied with a string instruction, because they address ES
 *     and ES is the kernel's (67.5.1);
...
static char sm_line[40];                 /* STATIC: os88_utoa() takes its
                                          ...
                                          * toolchain refuses to build (67.5) */
...
static void sm_draw(void *win)
{
     ...
     * tools/cc8086.py refused both of them by name (67.5). Statics are also
     * what keeps the frame small enough for a 256-byte worker stack (67.8) -
