...
| `wval.c` | the hostile-bundle reader (§2, §10.4) — **in the overlay** (§1.2) |
| `wflow.c` | the flow walk (§7) |
| `wpaint.c` | the component painter, the hit test and the per-component runtime state (§6) |
| `wact.c` | the press, the release, the field pool, focus and `^R` (§6.5–§6.8, §1.7) |
| `wevent.c` | the ring's doorway, the slices, the errors, the timer (§4.9–§4.11) |
| `wnative.c` | §6's get/set/method surface and §8.1's six impure builtins |
| `wstate.c` | §8.3's `.SAV`, the only file surface |
| `wovl.c` | `WEAVE.OVL`'s own tenants: About and Bundle Info |
| `wblob.inc` | the bundle claim's accessors (assembly) |
| `wdraw.inc` | the paint and hit-test cores (assembly, §1.2's seam — LOOM's Preview paints with these) |
| `wui.inc` | the shared alert, the arm word, the `os88line` wrappers, and the bridge the bytecode core leaves through |
| `wvm.inc` | **the WJS VM** (§4). Named by nothing outside itself but `wvm_native`, which is what lets `hosttest/weavevm.asm` run it in a boot sector |
