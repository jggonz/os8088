...
Menus: **Apple**: "About jop..." (CMD_ABOUT), "Control Panel"
(CMD_CTRL, string `menu_s_ctrl`, §31), "Task Manager" (CMD_TASKS,
string `menu_s_tasks`, §28) — Apple's item count in `menu_table` is 3.
...
Apple = CMD_ABOUT, File = CMD_NOTE, Special = CMD_REBOOT.
...
and onto the end of the Apple menu therefore renumbered the File menu
...
item by `menu_widest`, so the Apple menu is as wide as "Control Panel" and
...
- **About** — 300×120 at (170,140), title "About jop". Paint: centered
  lines "jop 1.0", "a graphical OS for the 8086", and a third line whose
...
10. Apple → Task Manager opens a window with a live CPU load gauge and
...
11. Apple → Control Panel opens a singleton two-pane window: the left pane
...
Menu/dispatch: see §12/§13 — "Task Manager" (CMD_TASKS = 3) is the Apple
...
selected item, kept as initialised `.text` data (the `japi_seed` trick of
...
= leftmost pixel (the §12 apple / §25 icon convention): `cp_radio_off` (an
...
fits: `menu_apple_glyph` hardcodes its top row at MENU_APPLE_Y, and
