...
A **USB mouse** works in the Book8088's USB socket too, and has been run on
one: open the Control Panel's Drivers page, tick **USB Mouse**, and plug a
mouse in -- it is found when it is plugged, not only at boot. The socket is a
CH375 chip that the Book8088's BIOS uses for flash drives, so the driver
leaves a flash drive alone. Confirmed on a stock Book8088 with a wired mouse
(SPEC.md 9.12.4); a wireless receiver, and an ISA CH375 card in another
machine, are the two reports still wanted.
