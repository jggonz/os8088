#!/usr/bin/env python3
"""PIXELSTEIN 3D's fixed-point tables (SPEC.md 96.12), generated.
...
top two bits (SPEC.md 96.1) - and every trigonometric fact it needs is one of
...
that produced it.
  ...
  px_tan   the same quarter, Q8.8, CLAMPED AT 0x7FFF (127.996). It is the
           ...
           body (SPEC.md 96.2). A clamped entry means that walker crosses no
           line inside a 64-tile map and is PARKED (96.2.3): the clamp is the
...
ANG = 4096                      # angle units a turn (SPEC.md 96.1)
QUARTER = ANG // 4
SIN_ONE = 16384                 # Q14
TAN_ONE = 256                   # Q8.8
...
def park_index():
    """The first quarter-index whose tangent is clamped (SPEC.md 96.2.3)."""
    t = tan_table()
...
def generate():
    ...
    w(";")
    w("; GENERATED by tools/pxstab.py - do not edit by hand (SPEC.md 96.12).")
    ...
    w(";")
    w("; The 12-bit angle (SPEC.md 96.1): %d units a turn, the quadrant in the" % ANG)
    ...
    w("                                  ; clamped step PARKS its walker (96.2.3)")
