#!/usr/bin/env python3
"""Generate apps/telnet/tecp437.inc - the CP437 glyphs the machine cannot be
asked for.
...
argue with in a review.
  ...
  python3 tools/cp437font.py             rewrite apps/telnet/tecp437.inc
...
OUT = os.path.join(ROOT, "apps", "telnet", "tecp437.inc")
...
BANNER = """\
; tecp437.inc - the CP437 glyphs a machine cannot be asked for.
;
...
;
; SPEC.md 70.8.6 is the contract. The package builds te_glyf, 0..255, at
...
def build():
    out = [BANNER, "",
           "TE_CP437_N  equ 160              ; 0..31 then 128..255", "",
           "te_cp437:"]
...
def main():
    ...
    if "--check" in sys.argv:
        try:
            ...
            cur = None
        if cur != text:
            print("cp437font: apps/telnet/tecp437.inc is stale - run "
                  "tools/cp437font.py")
            if cur is not None:
    ...
    with open(OUT, "w", encoding="utf-8", newline="\n") as f:
        f.write(text)
    print("cp437font: wrote apps/telnet/tecp437.inc (160 glyphs, 1,280 bytes)")
    return 0
