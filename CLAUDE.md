...
- `tests/` — **testing apps, and the only packages built from outside `apps/`.** `fontbench/` prices the primitive (SPEC.md §6.1.1), `typebench/` the keystroke (§11.94). `all` never builds them, nothing under it is tracked, and none of it reaches a shipped floppy: `make bench` is the on-demand target, and `make test TESTAPPS=build/bench.img` boots one. Leaving the artifacts untracked is what keeps them out of `all` — `make check-images` reads `git ls-files build`, so tracking one would force it back into `all` or read as ORPHAN. The **`testing` branch** is where these are *developed*, so their iteration (two of three corrections so far were to the apparatus, not the thing measured) stays out of this history; a finished harness lands here. docs/TESTING.md.
