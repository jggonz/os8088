...
- **Near model — for the kernel.** CS = DS = `KERNEL_SEG` (0x0800) for all kernel code and every task; **SS = `LOW_SEG`** (0x0440), because every task stack lives outside the kernel segment. Inter-module calls inside `.text` are near; `.fartext` modules go through the shims in SPEC.md §33. ES is scratch but must be restored unless documented. **SS ≠ DS means `[bp+disp]` addresses SS** — code holding a kernel pointer in BP needs `[ds:bp+…]`. **A package owns its own segment** (SPEC.md §20.1), so every crossing of that boundary is a far call in one direction and a dispatcher call in the other; see "Packages own a segment" below.
...
- **Memory budget.** Kernel image + .bss must fit below `KERN_MAX` = 0xB000, and so must image + .fartext; `kernel.asm` ends with build-time assertions that fail the build if exceeded (SPEC.md §15.1). Measure with the recipe there before assuming headroom. The package pool left the kernel segment entirely (`PKG_SEG`, 60KB), and everything above it is the **claim heap** (SPEC.md §50) — nothing up there has a fixed address. `docs/MEMORY-PLAN.md` records the whole ladder and how it got that way. Below the kernel the ladder is *sized*, not reserved: guard 3 keeps 4KB under task 0 and guard 8 keeps `KERNEL_SEG` off the boot sector, but nothing catches a task stack that outgrows its 512-byte slice — re-run the fill probe (MEMORY-PLAN step E) before trusting a smaller number.
...
Three one-line hooks move it, and nothing else in the kernel knows the bar exists: `wm_front` activates the window it raises (so launching, raising, un-minimizing and dock clicks all follow for free); the event ladder's window branch activates the clicked window too (a click on the *already* frontmost window never reaches `wm_front`, and the bar still has to follow); and `menu_check`, run at the top of every `menu_draw_bar`, hands the bar to `wm_top` the moment `[menu_win]` names a window that stopped being visible — one validation covering close, minimize and hide. It **promotes rather than reverting** because the title bar does: losing the front window promotes whatever was under it and `wm_paint_dmg` gives that window the pinstripes (§11.5), so a bar that fell back to Locator instead made the screen say two different things about which app is active. `wm_top` answers 0 when nothing visible is left, and 0 *is* Locator, so the old fallback is still the last rung. A deliberate switch to Locator (clicking the bare desktop) is sticky — `[menu_win]` = 0 leaves `menu_check` at its first test.
...
The kernel's 64KB segment is not all the kernel's. Four moves bought it room, and a fifth bought the *heap* room by shrinking what is below the kernel:

- **Task stacks and disk buffers left the segment** into `LOW_SEG` (linear 0x04400), 9KB of `.lowbss` — 11 × 512-byte task stacks plus the disk buffers, sized against a measured 150-byte high-water mark rather than guessed. Consequence: SS ≠ DS. The disk buffers are read only through `dsk_get_dir`/`dsk_get_icon`, which stage one entry back into the kernel segment so every consumer keeps a plain DS:SI pointer.
- **The package pool left the segment altogether** (SPEC.md §20.1): `PKG_SEG` = `KERNEL_SEG` + `KERN_MAX`/16, `PKG_PARA` paragraphs of it, one segment per loaded package. `APP_MAX_SIZE` is mirrored in `kernel/kernel.asm`, `apps/os88api.inc` and `tools/os88pkg.py` — change them together and rebuild every `.o88`.
- **The file manager's listings left the segment too** (SPEC.md §2.3/§22.1): each open Disk window owns a `VIEW_KB` (3KB) **heap claim** — 1KB of directory entries plus 2KB of icons, a byte-for-byte copy of `disk_dir` + `disk_icons`, reached through the `FS_VSEG` segment its state block carries. That is what makes several file-manager windows affordable: **paints read the window's cache, actions re-sync the global snapshot first** (`fmv_sync`), so a repaint, a drag or a `wm_paint_all` costs zero floppy I/O. It was four pinned 4KB slots at `VIEW_SEG` = 0x2C00 until §50 made it claimable, so a machine with no Disk window open now pays nothing for it; `docs/MEMORY-PLAN.md` Step D records the 16KB it used to cost.
- **The kernel itself moved down** to `KERNEL_SEG` = **0x0800** (SPEC.md §2), because once the four rungs below it were *measured* they came to 30,464 bytes, not the 62,976 that had been set aside — and the 32KB difference was memory nothing could address. The floor is the boot sector: `boot/boot.asm` runs at 0000:7C00 and is *still running* while the kernel's sectors land (it far-calls the splash after each one), so `KERNEL_SEG` can never go below 0x07E0; guard 8 asserts it. **Three files carry the constant** — `kernel/kernel.asm`, `boot/boot.asm` and `apps/os88api.inc`, the last because it is baked into every package's far-call targets, so a kernel move means rebuilding every `.o88` and both apps floppies. The measurement (a 0xCC fill of low memory plus a hard workout) is the only reason to believe any of the reservations, and `docs/MEMORY-PLAN.md` step E records both it and what it cost: `DSK_FAT_SECS` = 10 puts every FAT16 volume below mount rule 10's threshold, so the 2.88MB test geometry and the `filetest-fat16.img` target are gone and the FAT16 decode is dead code.
...
Everything above the package pool is handed out on demand. `mem_claim` takes KB and an owner word and answers a segment; `mem_free`/`mem_free_owner`/`mem_free_rec` give it back, and `mem_avail` reports the largest free run and the total. Four things about it are load-bearing:
...
- **The kernel is a client too**, with its own owner tags: the menu save-under (`MEM_K_SAVE`) and the back buffer (`MEM_K_BB`). `bb_set` claims 150KB when double buffering is armed and frees it when it is switched off, and the Control Panel's Display row **greys out with "Not Enough Ram"** when `bb_canfit` says the heap cannot fund it right now. That is live state — open Paint and the row greys, close it and it comes back.
