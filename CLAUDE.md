...
- `kernel/font.inc` — the 8x8 text renderers. `font_char` is transparent-background and clips per whole cell; **`font_run` (API 0x0240, SPEC.md §6.1) is the erase-and-letter pair as ONE operation** — both colours, each cell painted complete. Two things that buys: it cannot produce §11.3's granularity failure (fill and glyph are one decision about one cell), and on a 1bpp adapter at a byte-aligned x a cell row is a single store — no shift, no read, no second byte, no separate fill pass. The fast path needs `[bb_on]` **and** `x & 7 == 0` **and** `[gfx_dis]` clear; anything else falls back to `gfx_fill` + `font_str`, which is what every caller wrote by hand, so it is never slower than doing it manually. Tracker's pattern columns were moved onto 8-pixel boundaries to earn it.
