...
- `kernel/font.inc` — the 8x8 text renderers. `font_char` is transparent-background and clips per whole cell; **`font_run` (API 0x0258, SPEC.md §6.1) is the erase-and-letter pair as ONE operation** — both colours, each cell painted complete. It is a SPEED optimisation for the mono adapters and is measured, not asserted: `apps/fontbench` says **1.26x fewer instructions and 2.85x less framebuffer traffic** (228 byte-accesses down to 80, and the reads go to zero) for a ten-character run on Hercules and on CGA alike. On a 1bpp adapter at a byte-aligned x the cell owns its whole framebuffer byte, so a cell row is a single store — no shift, no read, no second byte, no separate fill pass. **Two things about it are commonly got wrong.** The fast path needs `[bb_on]` **and** `x & 7 == 0` **and** `[gfx_dis]` clear, and anything else falls back to `gfx_fill` + `font_str` — which costs **2.5% MORE** than writing that pair by hand (the far call, the gates, `font_width_x`), so it is not free on VGA and the tracker calls it on mono only. And the "cannot produce §11.3's granularity failure" property belongs to the FAST PATH, not to the call: the fallback is literally the fill-then-letter pair, so under an armed clip region it blanks a cut line exactly like hand-written code, and a caller that is not certain of both conditions still owes the region a `wm_clip_test` gate (SPEC.md §6.1.2). Tracker's pattern columns were moved onto 8-pixel boundaries to earn it, and it is the only consumer because it is fullscreen — a draggable window's content x is arbitrary mod 8 and re-rolls on every drag.
