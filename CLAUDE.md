...
- `mouse.py down` silently ignores coordinate arguments — it presses at the CURRENT cursor position. Always `mouse.py to X Y` first; a `down X Y` that lands somewhere else looks exactly like a kernel bug.
