...
- **The package pool has its own address space** (SPEC.md §20.1): `PKG_SEG` = wherever the kernel actually ends, `PKG_PARA` paragraphs of it, one segment per loaded package. `APP_MAX_SIZE` is mirrored in `kernel/kernel.asm`, `apps/os88api.inc` and `tools/os88pkg.py` — change them together and rebuild every `.o88`.
