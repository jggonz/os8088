...
- **The granularity rule, which is the sharp edge.** Fills clip per pixel and glyphs clip per whole cell, so **anything that erases a rect and then draws text into it must not let the two disagree**. Ungated, a window cut horizontally by another window's edge gets its visible rows white-filled and then no text back in them — it goes *blank*, not stale, and re-blanks on every update. Two ways out, both in the tree: erase per cell behind a `wm_clip_test` on that cell (`app_clk_render`), or gate the whole erase+draw pair on a `wm_clip_test` of the whole rect and skip both (`fr_status`).
  ...
  **The whole-rect gate is charged to the wrong thing when the cut is VERTICAL**, and the Task Manager is where that showed: a window overlapping the list's right-hand columns cuts exactly one glyph cell per row, and the gate threw away every row to protect it — so a partly-covered Task Manager stopped listing anything new, and an app launched while it was covered never appeared until something forced a full repaint. `tm_row_cells` is the fallback, and **the content check comes first**: `tm_rowsum`/`tm_elchk` decide whether the row changed at all, whatever the clip says, so an occluded Task Manager costs one 20-byte hash per row per refresh rather than a redraw per row — getting that order wrong is a real regression on a 4.77MHz machine, where the per-cell path is 20 clip tests, fills and glyphs. Only a row that actually changed then asks whether the whole band can go at once, and falls back to **per cell** if not. A per-cell pass that draws *nothing* (every cell cut) zeroes the check word again, so the row is retried rather than marked clean while blank. A horizontal cut still fails every cell and so still draws nothing, which is what the gate was right about. Two things the per-cell path must not forget, because the band fill used to do them: the lead-in before the first glyph and the tail past the last one hold no glyphs, so they are plain fills — safe at any clip, since fills clip per pixel — and without them a character from an older, longer row survives beside the text forever. `wm_clip_test` is API slot 0x0180 for exactly this. Solid-only drawing is unaffected — Bounce erases and redraws with `gfx_fill` at both ends.
