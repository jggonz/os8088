...
**SPEC.md §67 is the contract.** It says what is true and why. This file says
what to type, in what order, and what to do when the build refuses you. If the
two ever disagree, §67 is right and this file is stale.
...
says what to write instead. §67.5, §67.5.1, §67.7 and §67.8 are the reasoning;
...
normal; raising it is an argument to have in §67.8.
...
- **`os88pkg.py` reporting a size mismatch does not mean a truncated file.** On
  ...
  *current* section, so in a four-section file the header's image-size word
  ...
  layout exists to make structurally impossible. §67.2 has the full account,
