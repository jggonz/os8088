...
| **2 Textured** — the compiled scalers, 32-texel columns, dither phase (§13.1) | as §3 | **8086 (the XT): 64 × 80, Auto** | parts 3/4/5 |
...
- **Detail ▸ Resolution: Full / Low.** Full = one ray per shadow byte (64 rays across the 256-px CGA4 view; the plan as written). **Low = one ray per TWO shadow bytes** — 32 rays across the SAME view, every column drawn 8 CGA pixels (16 Hercules dots, 8 Mode X pixels) wide. The scaler set for Low is generated with a WORD store per row (`mov [di+r×80],ax`, AL = AH, 4 bytes, ~29 clk on the 8088 — one instruction where Full uses one, so a Low frame's STORE count per screen area is ~58 % of Full's) and the Δ-fill and present are unchanged (the same bytes reach the glass). The cast halves (156,500 → ~78,000), the texel loads halve, the sprite posts halve. Rows stay 80 (a Low-rows arm — 40 texel rows doubled — is NOT taken: the scaler already emits one store per screen row and halving the loads saves ~2 % of the frame for a picture that reads as bands).
- **Priced (D, CGA4, scene A)**: Textured Low ≈ 602,000 − 78,000 − 49,000 − 20,000 (stores at 58 %) ≈ **455,000 = 10.5 fps**; Flat Low ≈ **11.5**; Wire Low ≈ 12+ (cast-bound). These are the numbers §96 quotes beside Full's, and `tests/pixelstein.py` reports Low on every machine it runs on (fullscreen; §15).
- **Auto's order** (§14 amended): Textured Full → **Textured Low** → Flat Full → Flat Low → Wire. Halving the rays is the first thing a slow machine gives up, because a wall that is still textured at 32 columns reads better than a flat one at 64 (Wolf's own `viewsize` trade). The pinned rung and resolution persist in `PXSTEIN.CFG`.
- **The menu, in full**: *Mode* (the adapter's fsx items, §2) · *Detail ▸ Auto / Wire / Flat / Textured* (Textured greyed with its fact when part 3/4/5 is refused: "Textured: needs 80 KB free") · *Resolution ▸ Full / Low* · *Size* (view width 48–80 columns, §2) · *Rows* (80/100, where the surface allows) · *Sound* · *Mouse*. Every item is a fact-greyed §47 control; none is a guess.
- **Part 3 holds BOTH scaler sets** (Full's byte-store set and Low's word-store set, ~30 + ~30 KB → the contiguous claim grows by ~30 KB, ~162 KB / 177 on VGA) so a resolution switch regenerates nothing and Auto can step between frames; if the claim is refused whole, `OP_OPT` degrades to Flat/Wire as before, where resolution is a store-width flag on the static ladders (a second static ladder set, +1.3 KB image). `pxsgen.py` models both sets; `pxsscale.py` diffs both. Wave 1 builds the Resolution row for the static ladders (Wire and Flat, both widths); wave 2 adds the word-store textured set.
