...
| MFM / ST-11M | **86Box, or MartyPC for CORRECTNESS only** — it has `IbmXebec`, and rung 0 is an option ROM either way, but MartyPC's **hard-disk** timing is unmodelled, so no timing from it counts here. (PERFORMANCE.md Sets 35/37 gave the FLOPPY a platter and it is now within a measurement quantum of the field machine; `tools/martypc/patches/04-floppy-disk-timing.patch` touches `fdc/` and `floppy_drive.rs` only, and nothing under `hdc/`, so an XT-IDE or Xebec transfer still arrives the instant it is asked for) | it ships the XT ST-506 family (IBM/Xebec, DTC 5150X, WD1002A-WX1, and the Seagate ST-11M/R). Confirm the exact `hdc =` key with the launch-and-`kill -TERM`-and-read-back trick in CLAUDE.md — 86Box rewrites its config with what it actually accepted |
