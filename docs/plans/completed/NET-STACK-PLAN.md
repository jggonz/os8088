...
| # | build | proves |
|---|---|---|
| **A** | ✅ **BUILT** — `OSAPI_DRV_CALL` (slot 0x0348) + `DSV_PKGCALL`, two verbs in `RAMDISK.DRV`, `tests/drvcall` | a package reaching a driver at all — testable in a container before any networking exists. SPEC.md §20.11. Measured: `.text` +103, `.bss` +10 (estimated ~58; the extra is the fence and the class arithmetic). **No rung on `kern_big`; one on `kern_small`, which is now at 0 spare** — its rung had two bytes left, so the next change to that build owes a decision either way |
