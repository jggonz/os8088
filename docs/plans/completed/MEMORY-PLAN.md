...
Forward-looking in `docs/history/SOUND-PLAN.md`'s sense for the rest: it is the
argument and the costing, written down before the code so the decisions are
the ones we meant to take.

It starts from two things measured in docs/plans/completed/NOTEPAD-NOTES.md §7: raising an
...
### Carried over from docs/plans/completed/NOTEPAD-NOTES.md §7
